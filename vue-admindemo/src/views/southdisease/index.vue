      ...
      <div class="southdisease-content tablecontent">
        <el-table :data="tableData" border style="width: 100%" :header-cell-style="{background:'#f6f6f6',color:'#333333'}" v-loading="isloading">
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button size="mini" type="text" @click="operation(scope.row.id, 'detail',scope.row.taskNumber)" v-if="permission1_2_1||isAdmin">查看</el-button>
              <el-button size="mini" type="text" @click="operation(scope.row.id, 'artificialdistribution',scope.row.taskNumber)" v-if="(permission1_2_2||isAdmin)&&scope.row.status==4">人工分配</el-button>
              <el-button size="mini" type="text" @click="operation(scope.row.id, 'stop',scope.row.taskNumber)" v-if="(permission1_2_3||isAdmin)&&(scope.row.status==4||scope.row.status==5||scope.row.status==8)&scope.row.isStop==1">暂停</el-button>
              <el-button size="mini" type="text" @click="operation(scope.row.id, 'run',scope.row.taskNumber)" v-if="(permission1_2_4||isAdmin)&&(scope.row.status==4||scope.row.status==5||scope.row.status==8)&scope.row.isStop==2">开启</el-button>
              <el-button size="mini" type="text" @click="operation(scope.row.id, 'end',scope.row.taskNumber)" v-if="(permission1_2_5||isAdmin)&&scope.row.status==7">结束</el-button>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    ...
    operation: function(taskId, name, taskNumber) {
      //操作
      if (name == "detail") {
