<template>
  <div class="systemdefaultproblem  content-padding">
    ...
    <div class="systemdefaultproblem-main">
      <div class="systemdefaultproblem-content tablecontent">
        <el-table :data="tableData" border style="width: 100%" :header-cell-style="{background:'#fafafa'}" v-loading="isloading">
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button size="mini" type="text" @click="operation(scope.row.id,'edit',scope.row.subject)">编辑</el-button>
  ...
  methods: {
    ...
    operation(id, type, subject) {
      if (type == "edit") {
        ...
        this.$confirm("是否确定删除【" + subject + "】标注工具？", "", {
...
<style lang="less">
.systemdefaultproblem {
  height:100%;
  overflow: hidden;
  .el-tabs {
    height: 41px;
  }
  .systemdefaultproblem-main {
    border-top: none;
    position: relative;
    padding-bottom: 50px;
    .systemdefaultproblem-content {
      table {
        th,
        td {
          text-align: center;
