    ...
    <div class="projectHallmanafement-main">
      <el-row :gutter="12" class="projecthall-item" v-for="(item,index) in tableData" :key="index">
        ...
        <div class="projectopration">
          <span @click="operation(item.id,'detail',item.projectName)" class="optoperation_btn max_size">查 看</span>
          <br>
          <br>
          <br>
          <span @click="operation(item.id,'review',item.projectName)" class="optoperation_btn max_size" v-if="item.status==2&&userType!=2">审 核</span>
          <span @click="operation(item.id,'edit',item.projectName)" class="optoperation_btn max_size" v-if="item.status>=3&&item.status<=7&&userType!=2">编 辑</span>
          <span @click="operation(item.id,'down',item.projectName)" class="optoperation_btn max_size" v-if="item.status>7&&userType!=2">下载数据</span>
        </div>
      </el-row>
