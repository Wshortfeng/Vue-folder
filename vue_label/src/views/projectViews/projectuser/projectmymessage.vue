    ...
    <div class="projectmessage-list tablecontent ">
      <el-table :data="tableData" style="width: 1650px" v-loading="isloading" show-overflow-tooltip="true" border :header-cell-style="{background:'#f6f6f6',color:'#333333'}">
        ...
        <el-table-column label="单据号" width="220px">
          <template slot-scope="scope">
            <p @click="goTo(scope.row.objectId,scope.row.billType,scope.row.status,scope.row.projectId,scope.row.isRead,scope.row.id)" class="underlineColor" v-if="scope.row.billNo">{{scope.row.billNo}}</p>
						<p class="underlineColor" v-else>{{scope.row.billNo}}</p>
          </template>
        </el-table-column>
