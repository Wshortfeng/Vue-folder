...
function imageLabel(e) {
	...
	function a() {
		...
		function a() { //使得底层图片完全显示在视图中
			...
			vuethat.nowImageWidth = a.width()
			vuethat.nowImageHeight = a.height()
			$(".imageLabel-img").css({
				width: vuethat.nowImageWidth * vuethat.labelSize / 10,
				height: vuethat.nowImageHeight * vuethat.labelSize / 10,
			})
		...
		$(".imageLabel-content")[0].oncontextmenu = function() {
			...
			$.each(n.data, function(e, a) {
				s = $('<div class="imageLabel-imgdrop ' + (a.note ? "imageLabel-drop-has" : "") + '"><span class="imageLabel-imgdrop-font">' + (a.note || "") + '</span><div class="imageLable-i-s"></div></div>');
				for(var i = 0; i < 4; i++) s.find(".imageLable-i-s").append('<i class="imageLable-i">');
				if(n.shade)
					for(var t = 0; t < 4; t++) s.append('<em class="imageLable-em">');
				var l = a;
				s.css({
					left: 100 * (l.end_x - l.x > 0 ? l.x : l.end_x) + "%",
					top: 100 * (l.end_y - l.y > 0 ? l.y : l.end_y) + "%",
					width: 100 * Math.abs(l.end_x - l.x) + "%",
					height: 100 * Math.abs(l.end_y - l.y) + "%"
				...
				if(d = e.button, 2 != e.button) {
					...
					if($(e.target).hasClass("imageLabel-imgdrop")) {
						...
						if(vuethat.imageType == 1) {
							...
							s.css({
								left: "calc(" + (100 * o.x) + "% - 7px)",
								top: "calc(" + (100 * o.y) + "% - 7px)"
							})
							if(n.shade) {
						...
						var borderColor = vuethat.getRandomColor()
						s.css({
							borderColor: borderColor
						})
						s.attr("borderColor",borderColor)
//						vuethat.imageLabelChange()
			...
			$(".imageLabel-img-boxs").mousemove(function(e) {
				...
				if(b) {
					if(vuethat.imageType == 1 && vuethat.clickType == "") { // 点击的是图片移动
						...
						o.end_x = (e.clientX - t.x) / t.w
						o.end_y = (e.clientY - t.y) / t.h
						o.y < 0 && (o.y = 0)
						o.x < 0 && (o.x = 0)
						o.end_y < 0 && (o.end_y = 0)
						o.end_x < 0 && (o.end_x = 0)
						...
						if(vuethat.lineType == "1" || vuethat.lineType == "2" || vuethat.lineType == "3" || vuethat.lineType == "4") {
							s.css({
								left: 100 * (o.end_x - o.x > 0 ? o.x : o.end_x) + "%",
								top: 100 * o.y + "%",
								width: 100 * Math.abs(o.end_x - o.x) + "%"
							}).addClass("imageLabel-drop-move")
						} else if(vuethat.lineType == "5" || vuethat.lineType == "6" || vuethat.lineType == "7" || vuethat.lineType == "8") {
							s.css({
								left: 100 * o.x + "%",
								top: 100 * (o.end_y - o.y > 0 ? o.y : o.end_y) + "%",
								height: 100 * Math.abs(o.end_y - o.y) + "%"
							}).addClass("imageLabel-drop-move")
						}
						...
						if(p) {
							...
							o.end_x = c.end_x + (e.clientX - t.cx) / t.w
							o.y = c.y + (e.clientY - t.cy) / t.h
							o.end_y = o.y + 6;
						} else if(v) {
							var a = g.index();
							0 == a && (o.end_x = c.end_x + (e.clientX - t.cx) / t.w)
						...
						if(p) {
							...
							o.end_x = o.x + 6
							o.y = c.y + (e.clientY - t.cy) / t.h
							o.end_y = c.end_y + (e.clientY - t.cy) / t.h;
						} else if(v) {
							var a = g.index();
							0 == a && (o.y = c.y + (e.clientY - t.cy) / t.h)
							1 == a && (o.end_y = c.end_y + (e.clientY - t.cy) / t.h)
						}
						o.y < 0 && (o.y = 0)
						o.x < 0 && (o.x = 0)
						o.end_y < 0 && (o.end_y = 0)
						...
						if(p) {
							...
							1 == a && (o.end_x = c.end_x + (e.clientX - t.cx) / t.w, o.y = c.y + (e.clientY - t.cy) / t.h)
							2 == a && (o.end_x = c.end_x + (e.clientX - t.cx) / t.w, o.end_y = c.end_y + (e.clientY - t.cy) / t.h)
							3 == a && (o.x = c.x + (e.clientX - t.cx) / t.w, o.end_y = c.end_y + (e.clientY - t.cy) / t.h)
							4 == a && (o.y = c.y + (e.clientY - t.cy) / t.h)
							5 == a && (o.end_x = c.end_x + (e.clientX - t.cx) / t.w)
							6 == a && (o.end_y = c.end_y + (e.clientY - t.cy) / t.h)
				...
				if(b) {
					...
					if(vuethat.imageType == 1 || vuethat.clickType == "img") {
						...
						o.x < o.end_x ? (i.x = o.x, i.end_x = o.end_x) : (i.x = o.end_x, i.end_x = o.x)
						o.y < o.end_y ? (i.y = o.y, i.end_y = o.end_y) : (i.y = o.end_y, i.end_y = o.y)
		...
		y.find(".imageLabel-input-close").click(function() {
				...
				var borderColor = s.attr("borderColor")
				e.note = u.val()
				e.type = vuethat.imageType
				e.imageSrc = vuethat.imageSrc
				e.labelId = vuethat.labelId
				e.toolName = vuethat.toolName
				e.width = vuethat.naturalWidth
				e.height = vuethat.naturalHeight
				e.x_pixel = parseInt(vuethat.naturalWidth * e.x)
				e.y_pixel = parseInt(vuethat.naturalHeight * e.y)
				if(vuethat.imageType == 1 || vuethat.clickType == "img") {
					e.end_x = undefined
					e.end_y = undefined
					e.end_x_pixel = undefined
					e.end_y_pixel = undefined
				} else if(vuethat.lineType <= 4 || vuethat.clickType == "xian1") {
					e.color = borderColor
					e.end_y = e.y
					e.end_x_pixel = parseInt(vuethat.naturalHeight * e.end_x)
					e.end_y_pixel = parseInt(vuethat.naturalHeight * e.end_y)
				} else if((vuethat.lineType >= 5 && vuethat.lineType <= 8) || vuethat.clickType == "xian2") {
					e.color = borderColor
					e.end_x = e.x
					e.end_x_pixel = parseInt(vuethat.naturalHeight * e.end_x)
					e.end_y_pixel = parseInt(vuethat.naturalHeight * e.end_y)
					...
					e.color = borderColor
					e.end_x_pixel = parseInt(vuethat.naturalWidth * e.end_x)
					e.end_y_pixel = parseInt(vuethat.naturalHeight * e.end_y)
				}
			...
			$(".imageLabel-edit").click(function() {
				...
				n.editPop && (y.addClass("imageLabel-active").find("input").val(image_name.note),
					setTimeout(function() {
