...
const apis = {
    ...
    getAcceptanceBatchTaskList(pageNum, pageSize, id) { //获取项目验收批次列表
        return http.post({
    ...
    getBatchLabelList(pageNum, pageSize, id) { //标注工具里获取左侧任务列表批次查看
        return http.post({
    ...
    notPassTaskLabel(batchTaskId, batchLabelId, type) { //审核任务不通过
        return http.post({
